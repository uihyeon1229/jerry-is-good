...
### 🔥 추가 결과 — Qwen 2.5-1.5B Model Size Ablation (2026-04-22 실측, 문서 21 전문)

"30B에서 주 지표 불변은 데이터 결함 아닌가?" 라는 예상 Q&A 에 답하기 위해 **변수 통제 ablation** 수행. **동일 데이터·epoch·LoRA 하이퍼파라미터**로 Qwen 2.5-1.5B 학습 + 벤치마크.

**유일한 변수 = 모델 크기.**

| 지표 | Nemotron 30B Δ | **Qwen 1.5B Δ** | 해석 |
|------|:---:|:---:|------|
| **expected_laws cov** | 0 (불변) | 🎯 **+0.067 (+27%)** | 30B 포화 지표가 1.5B에서 명확히 이동 |
| **면책 고지 포함률** | +25pp | 🔥 **+100pp** (0→100%) | 파이프라인 안전성 교육 신호 완전 전이 |
| 환각률 | 0→0 | 🛡️ **-5pp** (5%→0%) | L2 검증 필터가 SFT 후 유지 |
| 4단계 CoT 완전 준수 | 100% 포화 | **+15pp** (85→100%) | 구조 학습 신호 |
| 조문 인용 밀도 | +0.30 | +0.45 | 세부 개선 |

**SFT 속도**: Nemotron 30B 52분 vs **Qwen 1.5B 132초 (24× 빠름)**, train_loss 0.601.

**결론 (변수 통제로 입증)**:
- 우리 파이프라인 데이터는 **실제로 학습 신호로 작동**
- Nemotron 30B에서 주 지표 불변 = **데이터 결함 아니라 pretraining scale 포화**
- 1.5B(지식 얕음) → 동일 데이터의 상대 비중이 크므로 이동 가시적

**발표 슬라이드용 수치 3종 (21번 §7)**:
1. 🔥 면책 고지 **0% → 100% (+100pp)**
2. 🎯 expected_laws cov **0.242 → 0.308 (+27%)**
3. 🛡️ 환각률 **5% → 0% (-5pp)**
...
| **20** | **발표 경쟁력 평가 + 전략** | 레버 4종·Go/No-Go·백업 시나리오 |
| **21** | **🔥 Qwen 1.5B Model Size Ablation** | "데이터 결함 아님" Q&A 핵심 방어 |
