...
## 7-2. 페르소나 200명 대표 선별 (클러스터링)

> Q&A 대비: *"10,000명을 그냥 다 썼나요?"*

### 방법
- Nemotron-Personas-Korea 10,000명을 **bge-m3 임베딩** (연령·직업·학력·가족·지역·페르소나 요약 9개 필드 concat)
- **k-means 클러스터링 k=200** → 각 cluster centroid 최근접 1명씩 추출
- 결과: **200개 대표 납세 유형** — 1,000건 생성 시 각 대표 평균 5회 등장

### 왜 50 아니고 200?
- 50명은 한 사람당 20회 등장 → SFT 과적합 위험
- 200명은 한 사람당 5회 → **다양성·학습효율 균형**

### 왜 10,000 그대로 안 쓰나?
- 10K 무작위 추출은 중복·유사 페르소나 섞여 있음
- 대표 선별로 **커버리지 명시적 보장** + 발표 서사 강화

### 도구 재사용
- bge-m3는 이미 **B2 Semantic Drift**에서 사용 중 → 새 스택 부담 없음

### 발표 Q&A
> *"페르소나 10,000명을 bge-m3로 임베딩해 k=200 클러스터링하고 각 cluster 대표 1명씩을 골랐습니다. 1,000건 생성에 대해 한 페르소나당 평균 5회 등장해 **다양성과 학습 효율을 동시에** 확보했습니다."*
